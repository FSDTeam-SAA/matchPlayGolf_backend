  ...
  async getPlayerById(playerId, userId, userRole) {
    try {
      const player = await TournamentPlayer.findById(playerId)
        ...
        .populate('playerId', 'fullName email phone seeder clubName teamName')
        .populate({
          path: 'pairId',
          populate: [
            { path: 'player1', select: 'fullName email phone seeder clubName' },
            { path: 'player2', select: 'fullName email phone seeder clubName' },
          ],
        });
