...
class AdminDashboardService {
  ...
  async getOverview() {
    ...
    const totalPlayers = await RegisterUser.distinct("userId").then((ids) => ids.length);

    const ongoingMatches = await TournamentMatch.countDocuments({ status: "In Progress" });
    ...
    const recentRegistrations = await RegisterUser.find({})
      .sort({ createdAt: -1 })
      .limit(3)
